import { HeaderLayout, IHeaderLayoutProps } from '@dify-chat/components'

interface IPageLayoutProps {
	headerProps: IHeaderLayoutProps
	children: React.ReactNode
}

export default function PageLayout(props: IPageLayoutProps) {
	const { headerProps, children } = props
	return (
		<div className="h-screen relative overflow-hidden flex flex-col bg-theme-bg w-full">
			{/* 头部 */}
			<HeaderLayout {...headerProps} />
			<div className="flex-1 flex flex-col pt-5 bg-[#EDF4FF]">
				<div className="flex-1 bg-white rounded-t-[12px] py-6 overflow-y-auto box-border overflow-x-hidden flex items-center justify-center">
					{children}
				</div>
			</div>
		</div>
	)
}
